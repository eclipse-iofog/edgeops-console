...
import { decodeRegistryCa } from "@/lib/registryCa";

type Props = {
  data: string;
};

const RegistryCaTextBox: React.FC<Props> = ({ data }) => {
  ...
  const revealed = useMemo(() => decodeRegistryCa(data), [data]);
  const displayValue = visible ? revealed : data;
...
export default RegistryCaTextBox;
