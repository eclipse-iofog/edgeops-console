...
import { parseModelYaml } from "@/lib/yaml/parseModelYaml";
...
function Models() {
  const { data } = useData();
  const { items: models, loading: listLoading } = useResourceList("models");
  const modelsStore = useResourceStore("models");
  ...
  async function fetchModelItem(name: string) {
    try {
      setDetailFetching(true);
      const itemResponse = await request(`/api/v3/models/${name}`);
      if (!itemResponse.ok) {
        pushFeedback({
          message: controllerErrorMessage(itemResponse, "Failed to fetch model"),
          type: "error",
        });
      ...
      setSelectedModel(responseItem);
      const fogUuidsResponse = await request(`/api/v3/models/${name}/link`);
  ...
  const refreshFunctions = React.useMemo(() => {
    const map = new Map();
    map.set("Model", async () => {
      await modelsStore.fetch({ silent: true });
    });
    return map;
  }, [modelsStore]);
  ...
  const handleRefreshModel = async () => {
    ...
    try {
      const itemResponse = await request(
        `/api/v3/models/${selectedModel.name}`,
      );
      if (itemResponse.ok) {
        const responseItem = await itemResponse.json();
        setSelectedModel(responseItem);
      ...
      const fogUuidsResponse = await request(
        `/api/v3/models/${selectedModel.name}/link`,
      );
      if (fogUuidsResponse.ok) {
        const fogUuidsData = await fogUuidsResponse.json();
        setLinkedAgentItems(
      ...
      console.error("Error refreshing model data:", e);
    }
  };

  const attachModel = async () => {
    try {
      const res = await request(`/api/v3/models/${selectedModel.name}/link`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        pushFeedback({
          message: controllerErrorMessage(res, "Failed to attach model"),
          type: "error",
        });
        return;
      }
      pushFeedback({ message: "Model Attached", type: "success" });
  ...
  const detachModel = async () => {
    try {
      ...
      if (!res.ok) {
        pushFeedback({
          message: controllerErrorMessage(res, "Failed to detach model"),
          type: "error",
        });
        return;
      }
      pushFeedback({ message: "Model Detached", type: "success" });
  ...
  const handleDeleteModel = async () => {
    try {
      if (!selectedModel?.name) {
        pushFeedback({ message: "No model selected", type: "error" });
        return;
      }

      const res = await request(`/api/v3/models/${selectedModel.name}`, {
        method: "DELETE",
      });

      if (!res.ok) {
        pushFeedback({
          message: controllerErrorMessage(res, "Failed to delete model"),
          type: "error",
        });
      } else {
        pushFeedback({
          message: `Model ${selectedModel.name} deleted`,
          type: "success",
        });
  ...
  const handleEditYaml = () => {
    const spec: Record<string, unknown> = {
      ...
      registryId: selectedModel?.registryId,
    };
    if (selectedModel?.revision != null) {
      spec.revision = selectedModel.revision;
    }
    if (selectedModel?.format) {
      spec.format = selectedModel.format;
    }
    if (
      isHuggingFaceRegistry(selectedModel?.registryId) &&
      Array.isArray(selectedModel?.files)
    ) {
      spec.files = selectedModel.files;
    }
    ...
    addYamlSession({
      title: `Model YAML: ${selectedModel?.name}`,
      content: yamlString,
      isDirty: false,
      onSave: async (content: string) => {
        try {
          ...
          const [modelItem, err] = await parseModelYaml(parsedDoc);
          ...
          await handleYamlUpdate(modelItem);
        } catch (e: any) {
          pushFeedback({ message: e.message, type: "error", uuid: "error" });
        }
      },
  ...
  async function handleYamlUpdate(parsed: any) {
    try {
      ...
      if (res === null || !res.ok) {
        ...
        const updatedName = parsed.name || selectedModel?.name || "Model";
        pushFeedback({
          message: `Model: ${updatedName} Updated`,
          type: "success",
        });
        setIsOpen(false);
        await modelsStore.fetch({ silent: true });
    ...
    <>
      {showLoadingModal ? (
        <>
          <CustomLoadingModal
            open={true}
            message="Fetching AI Model List"
            spinnerSize="lg"
        ...
        <>
          ...
          <CustomActionModal
            ...
            title={`Attach ${selectedModel?.name}`}
          />

          <CustomActionModal
            open={showDetachModal}
            onConfirm={detachModel}
            onCancel={() => {
              setShowDetachModal(false);
              setAgentsToDetach([]);
            }}
            ...
            title={`Detach from ${selectedModel?.name}`}
          />
          ...
          <UnsavedChangesModal
            ...
            title={`Deleting Model ${selectedModel?.name}`}
            message={
              "This action will delete the model. If any microservices bind this model, the Controller will refuse the delete. This is not reversible."
            }
            cancelLabel={"Cancel"}
            confirmLabel={"Delete"}
          />
